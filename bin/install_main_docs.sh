...
grep -v " *#" ${SRC}/../contributors.csv | cut -f1-3 -d, | \
  sed -e 's/^/<tr><td>/' -e 's/,/<\/td><td>/g' -e 's/$/<\/td><\/tr>/' \
      ...
      >> ${DEST}/sponsors.html
echo "</table>" >> ${DEST}/sponsors.html
#echo '<p><i>Source list at: <a href="https://svn.osgeo.org/osgeo/livedvd/gisvm/branches/osgeolive_4/contributors.csv">https://svn.osgeo.org/osgeo/livedvd/gisvm/branches/osgeolive_4/contributors.csv</a></i></p>' >> ${DEST}/sponsors.html
